...
const AllProducts = () => {
  ...
  const handleTodaysDealToggle = (event, id) => {
    ...
    dispatch(toggleTodaysDeal(id, newCheckedState)).then(() => {
      enqueueSnackbar("Today's Deal updated successfully!", {
        variant: "success",
      });
    });
  };
  ...
  const [editProductId, setEditProdcutId] = useState(null);
  const [open, setOpen] = useState(false);

  const handleOpen = (productId) => {
    setOpen(true);
    setEditProdcutId(productId);
  };
  const handleClose = () => {
    setOpen(false);
    setEditProdcutId(null);
  };
                  ...
                  {datas.map((row, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell align="center">
                        <Switch
                          checked={
                            checkedStates[row?._id] || row?.todaysDeal || false
                          }
                      ...
                      <TableCell align="center">
                        <Switch
                          checked={
                            checkedStatesTwo[row?._id] || row?.featured || false
                          }
